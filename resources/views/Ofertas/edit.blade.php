...
<body>
    <div class="row">
        <div class="col-lg-6 margin-tb">
            <div class="pull-left">
                <h2>Editar Oferta con el id{{$Ofertas->id_oferta}}</h2>
            </div>
    ...
    <form action="{{url('Ofertas',$Ofertas->id_oferta)}}" method="POST">
        @csrf
        @method("PUT")
        ...
        <div class="row">
            ...
            <div class="col-xs-12 col-sm-12 col-md-12">
                <div class="form-group">
                    <strong style="font-size: 19pt">Cliente:</strong>
                    <select name="correo" id="correo">
                        <option style="font-size: 15pt" value="" disabled selected>Selecciona un Cliente</option>
                        @foreach($Clientes as $Cliente)
                            <option style="font-size: 15pt" value="{{$Cliente->id_cliente}}">{{$Cliente->correo}}</option>
                        @endforeach
                    </select>
                </div>
            </div>
            <div class="col-xs-12 col-sm-12 col-md-12">
                <div class="form-group">
                    ...
                    <select name="producto" id="producto">
                        <option value="" disabled selected style="font-size: 15pt">Selecciona un producto</option>
                        @foreach($Productos as $producto)
                            <option  style="font-size: 15pt" value="{{$producto->id_producto}}">{{$producto->nombre}}</option>
                        @endforeach
                    </select>
                </div>
            </div>
